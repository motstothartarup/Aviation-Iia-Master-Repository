...
# ---------- main ----------
def build_map(highlight_iatas=None, target_iata=None) -> folium.Map:
    """
    Return a folium.Map for ACA airports in the Americas.

    highlight_iatas: optional set/list of IATA codes to emphasize.
      - Target airport is taken from target_iata when provided.
      - If target_iata is not provided, we fall back to the grid HTML target.
    ...
    # Mirror ACA table logic: explicit target_iata wins, then parsed grid target, then first highlight
    target_iata = (target_iata or "").upper()
    if target_iata:
        chosen = target_iata
    elif parsed_target and parsed_target.upper() in highlight_list:
        chosen = parsed_target.upper()
    elif highlight_list:
        chosen = highlight_list[0]
    else:
        chosen = None
...
"""
    ...
    # JS: zoom meter + clustering (unchanged)
    js = r"""
(function(){
  try {
